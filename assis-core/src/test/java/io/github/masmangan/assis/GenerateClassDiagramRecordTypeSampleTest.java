...
class GenerateClassDiagramRecordTypeSampleTest {
	...
	void generatesRecordWithComponents() throws Exception {
		String puml = TestWorkbench.generatePumlFromSample("samples/records", tempDir, "records");
		...
		TestWorkbench.assertPumlContains(puml, "\"samples.records.Outer\" +-- \"samples.records.Outer$Inner\"");

		// association edge from record R to Outer$Inner with role "x"
		TestWorkbench.assertAnyLineContainsAll(puml, "\"samples.records.R\"", "--->", "\"samples.records.Outer$Inner\"",
				"x");
	}
}